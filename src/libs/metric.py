...
def rmse_lab(imtarget: np.ndarray, imoutput: np.ndarray) -> float:
    imtarget = skcolor.rgb2lab(imtarget)
    imoutput = skcolor.rgb2lab(imoutput)
    rmse = np.mean((imtarget - imoutput) ** 2) # missing np.sqrt()
    return rmse


def calc_rmse_lab(gts: List[np.ndarray], preds: List[np.ndarray]) -> float:
    rmses: List[float] = []
