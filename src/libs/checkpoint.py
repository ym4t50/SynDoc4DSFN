...
def save_checkpoint(
    ...
    if epoch % save_period == 0:
        torch.save(save_states, os.path.join(result_path, f"checkpoint_{epoch}_.pth"))
        logger.debug("successfully saved the ckeckpoint by save_period.")
...
def save_checkpoint_generator_discriminator(
    ...
    if epoch % save_period == 0:
        torch.save(save_states, os.path.join(result_path, f"g_checkpoint_{epoch}_.pth"))
        logger.debug(
            f"successfully saved the generator's ckeckpoint every {save_period} epoch."
        )
    ...
    if epoch % save_period == 0:
        torch.save(save_states, os.path.join(result_path, f"d_checkpoint_{epoch}_.pth"))
        logger.debug(
            "successfully saved the discriminator's ckeckpoint every {save_period} epoch."
        )
...
def resume_generator_discriminator(
    resume_path: str,
    generator: nn.Module,
    discriminator: nn.Module,
    optimizerG: optim.Optimizer,
